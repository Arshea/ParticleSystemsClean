	...
	int ClientGame::initEngine(GameContext &context, PE::MemoryArena arena, EngineInitParams &engineParams)
{
	...
	*/
	...
	{
		Handle h("ParticleManager", sizeof(ParticleManager));
		context.m_pParticleManager = new (h) ParticleManager(context, arena, h);
		context.getParticleManager()->addDefaultComponents();
	}
